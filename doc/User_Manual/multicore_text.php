...
<P> Starting from March 2012 (revision 11906), Topographica supports
...
machines. Enabling OpenMP will not prevent Topographica from working
as normal on single core machines or on multicore machines that lack
the necessary OpenMP libraries and compiler support.

<H2>OpenMP in Topographica</H2>

Using OpenMP in Topographica is straightforward, requiring a single
flag to enable OpenMP and a single (optional) variable to control the
number of threads used. To get the most out of your multicore system,
it is important to understand that enabling OpenMP does not guarantee
the best possible performance: in some situations you may find using
multiple concurrent but single-threaded instances of Topographica will
...
OpenMP is enabled and disabled via the global "openmp" variable in the
Topographica environment using '-c openmp=True' at the commandline. To
...
system while the simulation is running. The default OpenMP behaviour
to use all available cores when enabled unless a lower number of
threads has been manually specified (as described shortly).
...
If you wish to use OpenMP with Topographica regularly, you will want
to enable OpenMP define the number of threads to use in one place.
...
<i>after</i> you specify '-c openmp=True'. If you try the following
...
benchmark you system for your own simulations.